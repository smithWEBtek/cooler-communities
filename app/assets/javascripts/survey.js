$(() => {
  ...
  surveyJSON.pages.map(page => {
    ...
    surveyCategoryNames.push(page.name);

    categorySurveyDiv.Survey({
      model: categorySurvey,
      onComplete: saveCategoryResults
    });
    categoryTabsDiv.prepend(`<img src="/assets/images/${page.name}.png" class="survey__category-image" />`)
  });


  console.log('surveyCategoryNames: ', surveyCategoryNames);


  function saveCategoryResults(surveyCategoryResults) {
    let categoryName = Object.keys(surveyCategoryResults.data)[0];
    console.log("results: ", JSON.stringify(surveyCategoryResults.data));
    $("#surveyContainer").text(JSON.stringify(surveyCategoryResults.data));
    $.post({
      url: '/responses',
      dataType: 'json',
      data: surveyCategoryResults.data,
    }).done(function (results) {
      console.log('server results: ', results);
    })

    thankyouMessage(categoryName);
  }

  function thankyouMessage(categoryName) {
    categoryName = categoryName.toLowerCase()
      .split(' ')
      .map((s) => s.charAt(0).toUpperCase() + s.substring(1))
      .join(' ');
    $('div.sv_body.sv_completed_page')[0].children[0].innerText = `Thanks for completing the ${categoryName} category!`
  }
});
